...
# Applies to Makefile backend targets, with-tauri.sh, and build-macos.sh.
# Set CLIPGLASS_DISABLE_SCCACHE=1 for a clean rustc path (e.g. reproducible release debugging).
# This is intentionally opt-in by installation: machines without sccache keep Cargo defaults.
if [[ -z "${RUSTC_WRAPPER:-}" && "${CLIPGLASS_DISABLE_SCCACHE:-0}" != "1" ]] && command -v sccache >/dev/null 2>&1; then
