...
<div class="settings-shell">
    ...
    {:else if activePane === "voice"}
      ...
      <section class="settings-section">
        ...
        <label class="settings-toggle">
          ...
          />
          <span>Enable voice transcription shortcut</span>
        </label>
      ...
      <section class="settings-section" class:disabled={!settings.voice_transcription_enabled}>
  ...
  <section class="settings-section">
    <div class="settings-section-title">AI tagging</div>
    <label class="settings-toggle">
      <input
        type="checkbox"
      ...
      />
      <span>Automatically tag clipboard text entries</span>
    </label>
    <div class="settings-hint" style="margin-top: 8px;">
      When off, new clipboard entries are not auto-tagged (manual retag still works).
    </div>
  </section>

  <section class="settings-section" class:disabled={!settings.ai_tagging_enabled}>
  ...
  <section class="settings-section">
    ...
    <label class="settings-toggle" style="margin-top: 12px;">
      <input
        ...
        onchange={(e) =>
          void handleOverlayShortcutHintsToggle((e.currentTarget as HTMLInputElement).checked)}
      />
      <span>Show keyboard shortcut hints along the bottom of the clipboard panel</span>
    </label>
  </section>
  ...
  <section class="settings-section">
        ...
        <div class="excluded-apps">
          {#each excludedApps as app}
            <div class="excluded-app-row">
              <span class="excluded-app-name">{app.displayName}</span>
              <button
    ...
    {:else}
      <div class="settings-hint">App exclusion is available on macOS only.</div>
    {/if}
  </section>
...
<style>
  ...
  .sidebar-dot.ok {
    background: #0f8;
    box-shadow: 0 0 8px rgb(0 255 136 / 55%);
  }
  ...
  .dirty-save {
    min-height: 36px;
    padding: 0 18px;
    background: linear-gradient(180deg, rgb(16 185 129 / 95%), rgb(5 150 105 / 95%));
  ...
  .settings-select {
    ...
    padding-right: 42px;
    background-image:
      linear-gradient(45deg, transparent 50%, rgb(237 241 248 / 90%) 50%),
      linear-gradient(135deg, rgb(237 241 248 / 90%) 50%, transparent 50%),
  ...
  .settings-item.danger:hover {
    ...
    border-color: rgb(255 107 107 / 14%);
  }
  ...
  .status-dot.ok {
    background: #4ade80;
    box-shadow: 0 0 6px rgb(74 222 128 / 40%);
  }

  .status-dot.fail {
    background: #f87171;
    box-shadow: 0 0 6px rgb(248 113 113 / 40%);
  }
