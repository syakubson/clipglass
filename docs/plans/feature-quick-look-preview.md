...
- **Navigate:** while preview is open, `←` / `→` change the selected entry and refresh preview content (Finder behavior).
...
- **Content by type:**
  ...
  - _Image_ — largest available asset (thumbnail or full blob if stored); dimensions and file size in header.
...
2. **UI:** `QuickLookPanel.svelte` (or dedicated route `quicklook/+page.svelte` for the extra window) reusing `formatImageMeta`, `detectTextKind`, and tag display helpers from `ClipboardCard`.
...
- [ ] `←` / `→` with preview open updates content in place.
