...
- **Overlay filters** — two-row layout when AI tagging is on (Content Kind segment + tag chip bar); format-only row when tagging is off; dynamic panel height (420 / 440 / 480 px); settings sync on reveal; header close button and outside-click dismiss; search, content kind, and tag filters run in the database with DB-wide chip counts for the active scope.
