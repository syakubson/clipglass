...
- **Overlay filters** — tag chip bar (format chips always; semantic chips when AI tagging is on); static panel height from **Settings → Keyboard shortcuts** only (415 / 450 px); settings sync on reveal; header close button and outside-click dismiss; search and tag filters run in the database with DB-wide chip counts for the active scope.
...
- **Overlay** — Raycast-style open/close motion; `←/→` browse cards (arrows in search change selection, not cursor); empty states per filter type; SF Pro / SF Mono preview typography; filter chips visually distinct from card metadata tags; stale tag filters auto-clear when the grid is empty but history still has entries (`reconcileOverlayFilters`); History / Starred as a macOS segmented control (short labels); custom collections as scrollable pills; unified 32 pt header row (search, tabs, exclude, settings, close); **Exclude [App]** de-emphasized secondary chrome; **All / Text / Images** content-kind segment temporarily hidden pending user feedback (tag bar only).
- **Clipboard cards** — fixed-height preview and footer layout; Paste / Retag / Pin / Delete on hover or keyboard focus (not bare selection); pin star always visible with stronger warning card border and resting warning chrome on the star; distinct warning hover for pinned vs unpinned star; Delete hover/active danger chrome; SF Symbol action icons; success feedback only after a confirmed copy; pointer toolbar actions blur the card so focus does not stick; semantic footer tags use abbreviated labels with ellipsis and hover tooltip when clipped.
- **Semantic tag labels** — overlay filter chips and card footers abbreviate common tags (`javascript` → `js`, `typescript` → `ts`, `kubernetes` → `k8s`, …); synonymous stored tags merge into one chip with combined counts and filter together.
- **Iconography** — macOS SF Symbols replace custom stroke SVGs across the clipboard panel (search, header, card actions), settings section icons and controls, and voice HUD mic; native `<select>` chevrons use matching symbol paths.
